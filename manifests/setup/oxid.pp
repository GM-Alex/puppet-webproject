define webproject::setup::oxid (
  ...
  if $php_version == undef {
    $in_php_version = '5.3.28'
  } else {
    $in_php_version = $php_version
  }

  if $php_extensions == undef {
    $in_php_extensions = ['xdebug', 'zend', 'ioncube', 'xhprof']
  } else {
  ...
  file { "${project_dir}/config.local.inc.php":
    ensure => present,
    content => template("webproject/oxid-config.erb"),
  }
}